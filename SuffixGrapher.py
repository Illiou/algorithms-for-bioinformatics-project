...
class Grapher(object):
    ...
    def createGraphviz(self, step=""):
        ...
        string += "\tnode" + str(
            self.gst.root.id) + " [label=\"\",style=filled,fillcolor=lightgrey,shape=circle,width=.1,height=.1];\n"
        string += "//------leaves------\n"
    ...
    def _printInternalNodes(self, node, name="root"):
        ...
        if not node.is_root() and len(node.edges) > 0:
            string += "\tnode" + str(node.id) + "[label=\"" + str(
                node.id) + "\",style=filled,fillcolor=lightgrey,shape=circle,width=.07,height=.07]\n"
    ...
    def _printEdges(self, node, name="root"):
        ...
        for char, edge in node.edges.iteritems():
            ...
            if len(edge.l_v) > 0:
                edgeString = "{0} ({1})[{2}, {3}]{4}{5}".format(internalString, edge.string_id, edge.start, edge.end, edge.whole_length, edge.l_v)
            else:
                edgeString = "{0} ({1})[{2}, {3}]{4}".format(internalString, edge.string_id, edge.start, edge.end, edge.whole_length)
